...
for(R in 1:nr){
  beep(2)
  rho <- rhoseq[R]
  print(paste("sim g=0, h=0, rho =",rho))
...
for(R in 1:nr){
  beep(2)
  rho <- rhoseq[R]
  print(paste("sim g=0, h=0.2, rho =",rho))
  rho <- rngh.sub(50,g,h,rho)$rho.adjusted # adjust rho to get desired population value
  cmat1 <- matrix(c(1,rho,rho,1),2,2)
  cmat2 <- matrix(c(1,rho+es,rho+es,1),2,2)
  for(N in 1:length(nseq)){
    n <- nseq[N]
...
for(R in 1:nr){
  beep(2)
  rho <- rhoseq[R]
  print(paste("sim g=0.2, h=0, rho =",rho))
  rho <- rngh.sub(50,g,h,rho)$rho.adjusted # adjust rho to get desired population value
  cmat1 <- matrix(c(1,rho,rho,1),2,2)
  cmat2 <- matrix(c(1,rho+es,rho+es,1),2,2)
  for(N in 1:length(nseq)){
...
for(R in 1:nr){
  beep(2)
  rho <- rhoseq[R]
  print(paste("sim g=0.2, h=0.2, rho =",rho))
  rho <- rngh.sub(50,g,h,rho)$rho.adjusted # adjust rho to get desired population value
  cmat1 <- matrix(c(1,rho,rho,1),2,2)
  cmat2 <- matrix(c(1,rho+es,rho+es,1),2,2)
  for(N in 1:length(nseq)){
...
for(R in 1:nr){
  beep(2)
  rho <- rhoseq[R]
  print(paste("sim g=1, h=0, rho =",rho))
  rho <- rngh.sub(50,g,h,rho)$rho.adjusted # adjust rho to get desired population value
  cmat1 <- matrix(c(1,rho,rho,1),2,2)
  cmat2 <- matrix(c(1,rho+es,rho+es,1),2,2)
  for(S in 1:nsim){
...
for(R in 1:nr){
  beep(2)
  rho <- rhoseq[R]
  print(paste("sim g=0, h=0, rho1 =",rho))
  cmat1 <- matrix(c(1,rho,rho,1),2,2)
  cmat2 <- matrix(c(1,rho+es,rho+es,1),2,2)
  for(N in 1:length(nseq)){
    n <- nseq[N]
...
for(R in 1:nr){
  beep(2)
  rho <- rhoseq[R]
  print(paste("sim g=0, h=0.2, rho1 =",rho))
  rho <- rngh.sub(50,g,h,rho)$rho.adjusted # adjust rho to get desired population value
  cmat1 <- matrix(c(1,rho,rho,1),2,2)
  cmat2 <- matrix(c(1,rho+es,rho+es,1),2,2)
  for(N in 1:length(nseq)){
...
for(R in 1:nr){
  beep(2)
  rho <- rhoseq[R]
  print(paste("sim g=0.2, h=0, rho1 =",rho))
  rho <- rngh.sub(50,g,h,rho)$rho.adjusted # adjust rho to get desired population value
  cmat1 <- matrix(c(1,rho,rho,1),2,2)
  cmat2 <- matrix(c(1,rho+es,rho+es,1),2,2)
  for(N in 1:length(nseq)){
...
for(R in 1:nr){
  beep(2)
  rho <- rhoseq[R]
  print(paste("sim g=0.2, h=0.2, rho1 =",rho))
  rho <- rngh.sub(50,g,h,rho)$rho.adjusted # adjust rho to get desired population value
  cmat1 <- matrix(c(1,rho,rho,1),2,2)
  cmat2 <- matrix(c(1,rho+es,rho+es,1),2,2)
  for(N in 1:length(nseq)){
...
for(G in 1:ng){
  beep(2)
  g <- gseq[G]
  rho <- rngh.sub(50,g,h,rho.pop)$rho.adjusted # adjust rho to get desired population value
  cmat1 <- matrix(c(1,rho,rho,1),2,2)
  cmat2 <- matrix(c(1,rho+es,rho+es,1),2,2)
  print(paste("sim rho1 = rho2 = 0.3, g =",g))
  for(N in 1:length(nseq)){
    n <- nseq[N]
    for(S in 1:nsim){
      out1 <- ghmul(n, g=g, h=h, p=2, cmat=cmat1)  
...
for(G in 1:ng){
  ...
  cmat1 <- matrix(c(1,rho,rho,1),2,2)
  cmat2 <- matrix(c(1,rho+es,rho+es,1),2,2)
  print(paste("sim rho1 = 0.3, rho2 = 0.5, g =",g))
  for(N in 1:length(nseq)){
    n <- nseq[N]
    for(S in 1:nsim){
      out1 <- ghmul(n, g=g, h=h, p=2, cmat=cmat1)  
...
for(G in 1:ng){
  ...
  cmat1 <- matrix(c(1,rho,rho,1),2,2)
  cmat2 <- matrix(c(1,rho+es,rho+es,1),2,2)
  print(paste("sim rho1 = 0, rho2 = 0.6, g =",g))
  for(S in 1:nsim){
    # generate max n sample
    out1 <- ghmul(nmax, g=g, h=h, p=2, cmat=cmat1)  
    out2 <- ghmul(nmax, g=g, h=h, p=2, cmat=cmat2) 
...
pA <- p + ggtitle("Spearman + bootstrap")
pA
# ggsave("./figures/rho0_diff06_h0_varyg_spea.png", width = 20, height = 10, units = "cm")
```
...
pB <- p + ggtitle("Pearson + bootstrap")
pB
# ggsave("./figures/rho0_diff06_h0_varyg_boot.png", width = 20, height = 10, units = "cm")
```
...
pC <- p + ggtitle("Pearson + Fisher")
pC
# ggsave("./figures/rho0_diff06_h0_varyg_fish.png", width = 20, height = 10, units = "cm")
```
