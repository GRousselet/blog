---
title: "Correlation estimation: power"
...
To achieve at least 80% power given an expected population rho of 0.4, the minimum sample size is `r round(approx(y=nseq,x=res.power[5,],xout=0.8)$y)` observations.

To achieve at least 90% power given an expected population rho of 0.3, the minimum sample size is `r round(approx(y=nseq,x=res.power[4,],xout=0.9)$y)` observations.

Alternatively, for a given sample size and a desired power, we can determine the minimum effect size we can hope to detect. For instance, given n = 40 and a desired power of at least 90%, the minimum effect size we can detect is `r round(approx(y=rho.seq,x=res.power[,4],xout=0.9)$y, digits = 2)`.
...
If we correlate variable *X* with variable *Y*, heteroscedasticity means that the variance of *Y* depends on *X*. Wilcox considers this example:
...
Estimation is slightly biased (mean and median) for max sd.y = 2, and the sampling distributions seem to be negatively skewed. This should lead to lower power than in the homoscedastic case.
For max sd.y = 5 the skewness seeems to increase and the bias increases.
...
## Simulation with rho = 0 + bootstrap
...
for(iter.n in 1:Nn){
  ...
  for(iter.var in 1:Nvar){
    ...
    print(paste0("Var condition = ", iter.var))
...
res.power <- res.power / nsim
save(res.power,
     file = "./data/corr_power_hetero_rho0_pb.RData")
```

### Plot results
```{r}
# get data
load("./data/corr_power_hetero_rho0_pb.RData")
xlabels <- c("10","","30","","50","","70","","90","","150","200","300") 
...
# save figure
ggsave(filename='./figures/figure_power_hetero_rho0_pb.png',width=9,height=5) 
```

## Simulation with rho = 0.4 + bootstrap
...
res.power <- res.power / nsim
save(res.power,
     file = "./data/corr_power_hetero_rho04_pb.RData")
```
...
```{r}
# get data
load("./data/corr_power_hetero_rho04_pb.RData")
xlabels <- c("10","","30","","50","","70","","90","","150","200","300") 
...
# save figure
ggsave(filename='./figures/figure_power_hetero_rho04_pb.png',width=9,height=5) 
```
