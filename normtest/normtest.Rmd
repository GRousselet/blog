...
There are methods to generate pdfs using parametric approaches, but there is no simple implementation for g-and-h distributions, no straighforward equivalent to `dnorm` or `dlnorm`. The R package `gk` offers generalised g-and-h and g-and-k distributions, but these distributions do not contain the original Tukey g-and-h distributions as a special case. The package `tukeyGH` offers what we want, but produces weird wavy distortions near the ends of the distributions. So here to illustrate the pdfs, I simply create large samples, estimate their densities, and plot them.  
